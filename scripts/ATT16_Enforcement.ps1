...
.EXAMPLE
    .\ATT16_Enforcement.ps1 -ReadmePath C:\users.txt -DryRun
.EXAMPLE
    .\ATT16_Enforcement.ps1 -ReadmePath C:\users.txt -Execute
.EXAMPLE
    .\ATT16_Enforcement.ps1
...
# Same idea as Start-ProcessWithTimeout, but for PowerShell cmdlets rather
# than external executables - cmdlets have no process handle to kill, so a
# hung one (DISM-backed Windows Optional Feature cmdlets waiting on a locked
# CBS store; Defender cmdlets waiting on the network) can't be bounded by
# Start-Process. Runs the scriptblock in a background job and abandons it if
# it doesn't finish in time, so the main script can move on regardless.
function Invoke-WithTimeout {
    ...
    try {
        if (-not (Wait-Job -Job $job -Timeout $TimeoutSeconds)) {
            Write-Host "[WARN] Operation did not finish within $TimeoutSeconds seconds - abandoning it."
            throw "Operation timed out after $TimeoutSeconds seconds"
        }
        if ($job.State -eq 'Failed') {
            $jobError = ($job.ChildJobs[0].JobStateInfo.Reason).Message
            throw "Operation failed: $jobError"
        }
...
# -------------------------
# ENFORCE SYSTEM PASSWORD POLICY / LOCKOUT POLICY (ATT16)
# -------------------------
...
if ($dryRun) {
    ...
    try {
        $cfgPath = Join-Path $env:TEMP "att16_secpol_export.cfg"
        # secedit's /db argument is a SCRATCH workspace it uses to compute and
        # then apply the merged policy to the live system - it is not meant
        # to point at the real %WINDIR%\security\Database\secedit.sdb, which
        # is already open/locked by the OS's security subsystem. Pointing
        # /configure at that live file is what caused "exited with code 1"
        # (a generic open/write failure against a file already in use). Use a
        # dedicated working database instead; secedit still applies the
        # result to the live system regardless of which db path is used.
        $workDb = Join-Path $env:TEMP "att16_secpol_work.sdb"
        $logPath = Join-Path $env:TEMP "att16_secpol.log"
        ...
        if (Test-Path $workDb) { Remove-Item $workDb -Force -ErrorAction SilentlyContinue }
        $importExit = Start-ProcessWithTimeout -FilePath "secedit.exe" -ArgumentList "/configure /db `"$workDb`" /cfg `"$cfgPath`" /areas SECURITYPOLICY /log `"$logPath`" /quiet" -TimeoutSeconds 30
        if ($importExit -ne 0) {
            $logTail = if (Test-Path $logPath) { (Get-Content $logPath -Tail 15) -join " | " } else { "(no log produced)" }
            throw "secedit /configure exited with code $importExit - $logTail"
        }

        Remove-Item $cfgPath, $workDb, $logPath -Force -ErrorAction SilentlyContinue

        # secedit exiting 0 doesn't guarantee the setting actually took - verify
        # by re-exporting the NOW-live policy and reading the value back, so
        # any future failure is visible in the console instead of only showing
        # up later as a missing score item.
        $verifyCfgPath = Join-Path $env:TEMP "att16_secpol_verify.cfg"
        $verifyExit = Start-ProcessWithTimeout -FilePath "secedit.exe" -ArgumentList "/export /cfg `"$verifyCfgPath`" /areas SECURITYPOLICY /quiet" -TimeoutSeconds 30
        if ($verifyExit -eq 0 -and (Test-Path $verifyCfgPath)) {
            $verifyContent = Get-Content $verifyCfgPath
            $complexityLine = $verifyContent | Where-Object { $_ -match '(?im)^PasswordComplexity\s*=' }
            $clearTextLine = $verifyContent | Where-Object { $_ -match '(?im)^ClearTextPassword\s*=' }
            Remove-Item $verifyCfgPath -Force -ErrorAction SilentlyContinue

            if ($complexityLine -match '=\s*1' -and $clearTextLine -match '=\s*0') {
                Write-Host "[OK] Verified: password complexity is enabled and reversible encryption is disabled ($complexityLine / $clearTextLine)."
            } else {
                Write-Host "[ERROR] secedit reported success but the live policy does not show the expected values ($complexityLine / $clearTextLine) - investigate manually via secpol.msc."
            }
        } else {
            Write-Host "[WARN] Could not verify the applied policy (re-export failed) - check secpol.msc manually."
        }
    } catch {
...
foreach ($name in $unauthorizedSoftware) {
    ...
    # Some installers (Python.org's MSI bundle is the classic example) register
    # ONE umbrella bundle entry (an EXE bootstrapper) PLUS several individual
    # MSI sub-component entries (Core Interpreter, Standard Library, pip
    # Bootstrap, etc.) that all match the same search term. Uninstalling the
    # sub-components individually via msiexec /x corrupts the shared
    # component/reference-count state the bundle uninstaller depends on,
    # producing a cascade of 160x errors (confirmed in testing). When both a
    # bundle (non-msiexec) entry and MSI sub-component entries are present for
    # the same search term, uninstall ONLY the bundle entry - it's designed to
    # remove its own sub-components correctly, and touching the MSI entries
    # directly is unsupported by the vendor and actively harmful here.
    $bundleApps = $matchedApps | Where-Object { $_.UninstallString -notmatch "msiexec" }
    $msiSubApps = $matchedApps | Where-Object { $_.UninstallString -match "msiexec" }
    if ($bundleApps -and $msiSubApps) {
        Write-Host "[INFO] '$name' matched both a bundle uninstaller and $($msiSubApps.Count) MSI sub-component(s) - uninstalling only the bundle entry to avoid corrupting shared component state:"
        $msiSubApps | ForEach-Object { Write-Host "  [SKIP] $($_.DisplayName) (handled by the bundle uninstaller)" }
        $matchedApps = $bundleApps
    }
...
# -------------------------
# UPDATE BUSINESS SOFTWARE (winget first if available, but NEVER dependent on
# it - direct vendor-download fallback always runs too)
# -------------------------
# winget requires the App Installer package and (usually) a Microsoft Store
# connection, neither of which is guaranteed on a CyberPatriot image - it was
# confirmed absent on at least one real run. Treat it as a nice-to-have first
# attempt only; Chrome and Notepad++ are both scored answer-key items, so
# their update path below downloads directly from the vendor and does NOT
# depend on winget being present or working.
...
if ($winget) {
    ...
    Write-Host "[WARN] winget is not available on this system - falling through to direct vendor downloads below (this is expected and handled)."
}

# -------------------------
# CHROME: GUARANTEED MACHINE-WIDE UPDATE (winget-independent)
# -------------------------
# The answer key specifically penalizes Chrome not being at the default
# machine-wide location or not being current. winget may be unavailable, and
# even when present it can land Chrome under a user's AppData folder instead
# of Program Files. Always (not just when missing) download and run Google's
# official offline Enterprise MSI - msiexec upgrades an existing install in
# place, always targets the standard Program Files location, and doesn't
# depend on winget, the Microsoft Store, or Chrome's own background updater.
Write-Host "`n=== Guaranteeing Chrome Is Current and at the Default Machine-Wide Location ==="
...
if ($chromeAtStandardLocation) {
    Write-Host "[INFO] Chrome already present at $chromeAtStandardLocation - installing over it to guarantee the latest version."
} else {
    Write-Host "[INFO] Chrome not found at either standard machine-wide location - installing fresh."
}

if ($dryRun) {
    Write-Host "[DRY-RUN] Would download and silently install/upgrade GoogleChromeStandaloneEnterprise64.msi to guarantee a current, machine-wide install."
} else {
    try {
        ...
        Write-Host "[FIX] Installing/upgrading Chrome machine-wide via msiexec..."
        ...
        Write-Host "[OK] Chrome installed/upgraded machine-wide via offline Enterprise MSI."
    } catch {
        Write-Host "[ERROR] Failed to install/upgrade Chrome via offline MSI: $($_.Exception.Message)"
    }
}

# -------------------------
# NOTEPAD++: GUARANTEED UPDATE (winget-independent)
# -------------------------
# Same rationale as Chrome - pull the latest release directly from
# Notepad++'s official GitHub releases (the project's own distribution
# channel) and run its NSIS silent-install flag, instead of depending on
# winget being present.
Write-Host "`n=== Guaranteeing Notepad++ Is Current ==="

if ($dryRun) {
    Write-Host "[DRY-RUN] Would query the Notepad++ GitHub releases API and silently install the latest x64 installer."
} else {
    try {
        $nppRelease = Invoke-RestMethod -Uri "https://api.github.com/repos/notepad-plus-plus/notepad-plus-plus/releases/latest" -Headers @{ "User-Agent" = "PowerShell" } -TimeoutSec 30 -ErrorAction Stop
        $nppAsset = $nppRelease.assets | Where-Object { $_.name -match '^npp\.[\d.]+\.Installer\.x64\.exe$' } | Select-Object -First 1
        if (-not $nppAsset) { throw "Could not find an x64 installer asset in the latest release." }

        $nppInstallerPath = Join-Path $env:TEMP $nppAsset.name
        Write-Host "[FIX] Downloading Notepad++ $($nppRelease.tag_name)..."
        Invoke-WebRequest -Uri $nppAsset.browser_download_url -OutFile $nppInstallerPath -TimeoutSec 120 -UseBasicParsing -ErrorAction Stop

        Write-Host "[FIX] Installing Notepad++ silently..."
        $exitCode = Start-ProcessWithTimeout -FilePath $nppInstallerPath -ArgumentList "/S" -TimeoutSeconds 120
        if ($exitCode -ne 0) { throw "Notepad++ installer exited with code $exitCode" }

        Remove-Item $nppInstallerPath -Force -ErrorAction SilentlyContinue
        Write-Host "[OK] Notepad++ updated to $($nppRelease.tag_name)."
    } catch {
        Write-Host "[ERROR] Failed to update Notepad++ via GitHub releases: $($_.Exception.Message)"
    }
}

# -------------------------
# WIRESHARK: GUARANTEED UPDATE (winget-independent)
# -------------------------
Write-Host "`n=== Guaranteeing Wireshark Is Current ==="

if ($dryRun) {
    Write-Host "[DRY-RUN] Would look up the current Wireshark version from the official download page and silently install it."
} else {
    try {
        # Wireshark does not publish a stable "-latest.exe" alias - the
        # filename is versioned (e.g. Wireshark-4.6.7-x64.exe) and changes
        # with every release, so the version must be discovered dynamically
        # rather than hardcoded (a hardcoded link goes stale and 404s).
        $downloadPage = Invoke-WebRequest -Uri "https://www.wireshark.org/download.html" -TimeoutSec 20 -UseBasicParsing -ErrorAction Stop
        $wiresharkLink = $downloadPage.Links |
            Where-Object { $_.href -match 'win64/Wireshark-[\d.]+-x64\.exe$' } |
            Select-Object -First 1 -ExpandProperty href
        if (-not $wiresharkLink) { throw "Could not find a current Wireshark x64 installer link on the download page." }
        if ($wiresharkLink -notmatch '^https?://') { $wiresharkLink = "https://www.wireshark.org$wiresharkLink" }

        $wiresharkInstallerPath = Join-Path $env:TEMP (Split-Path $wiresharkLink -Leaf)
        Write-Host "[FIX] Downloading Wireshark from $wiresharkLink..."
        Invoke-WebRequest -Uri $wiresharkLink -OutFile $wiresharkInstallerPath -TimeoutSec 120 -UseBasicParsing -ErrorAction Stop

        Write-Host "[FIX] Installing Wireshark silently..."
        $exitCode = Start-ProcessWithTimeout -FilePath $wiresharkInstallerPath -ArgumentList "/S" -TimeoutSeconds 180
        if ($exitCode -ne 0) { throw "Wireshark installer exited with code $exitCode" }

        Remove-Item $wiresharkInstallerPath -Force -ErrorAction SilentlyContinue
        Write-Host "[OK] Wireshark updated."
    } catch {
        Write-Host "[ERROR] Failed to update Wireshark: $($_.Exception.Message)"
    }
}
...
foreach ($feature in $riskyFeatures) {
    # DISM-backed cmdlets have no process handle to bound with a timeout, and
    # are a well-known real-world hang source if the CBS store is locked by
    # another pending operation (e.g. a Windows Update install in progress).
    try {
        $state = Invoke-WithTimeout -TimeoutSeconds 60 -ScriptBlock { Get-WindowsOptionalFeature -Online -FeatureName $using:feature -ErrorAction SilentlyContinue }
    } catch {
        Write-Host "[WARN] Could not query feature '$feature' (possibly a locked CBS store): $($_.Exception.Message)"
        continue
    }
    if (-not $state) {
    ...
    if ($dryRun) {
        ...
        try {
            Invoke-WithTimeout -TimeoutSeconds 120 -ScriptBlock { Disable-WindowsOptionalFeature -Online -FeatureName $using:feature -NoRestart -ErrorAction Stop } | Out-Null
        } catch {
...
if (Get-Command Set-MpPreference -ErrorAction SilentlyContinue) {
    Invoke-Fix -Description "enable Windows Defender real-time protection" -Action {
        Invoke-WithTimeout -TimeoutSeconds 30 -ScriptBlock { Set-MpPreference -DisableRealtimeMonitoring $false -ErrorAction Stop }
    }
    Invoke-Fix -Description "enable Windows Defender cloud-delivered protection (MAPS)" -Action {
        Invoke-WithTimeout -TimeoutSeconds 30 -ScriptBlock { Set-MpPreference -MAPSReporting Advanced -ErrorAction Stop }
    }
    ...
    if (-not $dryRun) {
        try {
            # Reaches out to the network for signature data - bound with a
            # timeout so an unreachable update server can't hang the script.
            Invoke-WithTimeout -TimeoutSeconds 90 -ScriptBlock { Update-MpSignature -ErrorAction Stop }
...
foreach ($psv2Feature in @("MicrosoftWindowsPowerShellV2Root", "MicrosoftWindowsPowerShellV2")) {
    try {
        $state = Invoke-WithTimeout -TimeoutSeconds 60 -ScriptBlock { Get-WindowsOptionalFeature -Online -FeatureName $using:psv2Feature -ErrorAction SilentlyContinue }
    } catch {
        Write-Host "[WARN] Could not query feature '$psv2Feature' (possibly a locked CBS store): $($_.Exception.Message)"
        continue
    }
    ...
    if ($dryRun) {
        ...
        try {
            Invoke-WithTimeout -TimeoutSeconds 120 -ScriptBlock { Disable-WindowsOptionalFeature -Online -FeatureName $using:psv2Feature -NoRestart -ErrorAction Stop } | Out-Null
        } catch {
