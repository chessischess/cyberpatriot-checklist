...
.PARAMETER ReadmePath
    Path to the README user list file. If omitted, you'll be prompted for it.
.PARAMETER DryRun
...
.EXAMPLE
    .\ATT14_Enforcement.ps1 -ReadmePath C:\users.txt -DryRun
.EXAMPLE
    .\ATT14_Enforcement.ps1 -ReadmePath C:\users.txt -Execute
.EXAMPLE
            ...
            try {
                if ($app.UninstallString) {
                    ...
                    if ($uninstallString -match "msiexec") {
                        ...
                        # uninstaller isn't silent-capable this may require the
                        # operator to click through it manually.
                        Write-Host "[WARN] $($app.DisplayName) uses a non-MSI uninstaller - it may show an interactive prompt."
...
# -------------------------
Write-Host "`n=== Attempting to Update Business Software (Notepad++, Google Chrome) ==="
